...
//            model.addAttribute("produtos", produtoService.listAll()); //atualiza o modelo com a lista modificada
//            model.addAttribute("sucesso", "Produto adicionado com sucesso!"); //No thymeleaf "${sucesso}"

        } catch (Exception e) {

//            model.addAttribute("erro", "Erro ao atualizar o produto: " + e.getMessage());
...
//            model.addAttribute: Usado para passar dados diretamente para a view renderizada na mesma requisição.
//            redirectAttributes.addFlashAttribute: Usado para passar dados que precisam estar disponíveis após um redirecionamento
//                    (por exemplo, para exibir uma mensagem de confirmação ou erro).
        }
